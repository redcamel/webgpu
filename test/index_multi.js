...
async function init(glslang) {
	...
	async function aaaaaa(device) {
		...
		const alignedUniformBytes = Math.ceil(matrixSize / 256) * 256; // uniformBindGroup offset must be 256-byte aligned

		const uniformBuffer = device.createBuffer({
			size: MAX*2 * alignedUniformBytes + Float32Array.BYTES_PER_ELEMENT,
